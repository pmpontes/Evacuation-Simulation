...
Tendo sido amplamente utilizado em aplicações de simulação, considera-se de particular utilidade tendo em conta, por um lado, o foco emna modelar ção do comportamento social e, por outro, a recolha de métricas associadas às simulações realizadas. Por último, tem-se a vantagem de poder acompanhar, de forma visual, o decorrer da simulação.
...
O módulo \textit{Simulation} é responsável pela configuração da simulação, no que diz respeito às ferramentas usadas - \textit{Repast}, \textit{JADE} e \textit{SAJaS}.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.6]{simulation.jpg}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{environment.jpg}
...
\begin{lstlisting}[caption= Código \textit{Java}\, responsável pelo envio de um pedido de direções.]
/**
* makeRequest.
...
O agente a quem foram pedidas direções responde com uma mensagem \textit{INFORM} ou \textit{REFUSE} uma dada probabilidade.

\begin{lstlisting}[caption= Código \textit{Java}\, responsável pela receção de pedidos de direções.]
/**
...
\begin{lstlisting}[caption= Código \textit{Java}\, responsável pela receção de respostas a um pedido de direções.]
/**
* receiveReply.
* Attempts to receive a reply of type INFORM or REFUSE.
* Updates areaKnowledge to a percentage of the knowledge of the person answering, if it is greater than the current value.
*/
void receiveReply() {
...
A representação do espaço é feita com recurso a múltiplas estruturas de dados, nomeadamente, uma \textit{Grid} (classe disponibilizada pelo \textit{Repast} que conterá os agentes), um \textit{array} bidimensional de caracteres com as posições das saídas e das paredes/obstáculos e um \textit{array} bidimensional de inteiros como um mapa de distâncias. Um utilizador tem a liberdade de indicar, na interface da simulação, um ficheiro com a configuração do ambiente onde estarão necessáriamente definidas as posições das saídas e das paredes/obstáculos.

\begin{itemize}

\item \textbf{Mapa de obstáculos e saídas}

Este mapa é representado como um \textit{array} bidimensional com caracteres que representam as saídas, paredes/obstáculos ou espaços livres, nas posições em que estão definidas no ficheiro de configuração. Esta representação tanto é usada para a criação do mapa de distâncias como, em \textit{runtime}, para determinar se uma célula contem um obstáculo ou uma saída.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{map_text.png}
	\caption{Representação visual dos conteúdos do mapa de obstáculos e saídas.}
	\label{map}
\end{figure}

\item \textbf{Mapa de distâncias}

Antes da execução da simulação, no momento em que o mapa é carregado, é construído um mapa de distâncias até às saídas. 
...
\newpage
\begin{lstlisting}[caption= Pseudo-código que retrata o algoritmo usado para o preenchimento do mapa de distâncias.]
generateDistanceMap() {
...
Este pré-processamento do melhor caminho até à saída mais próxima, a partir de cada célula, é uma abordagem válida, tendo em conta que o ambiente é estático. Revela também ter um desempenho considerávelmente melhor do que se fosse usado um algoritmo de \textit{path finding} a cada tentativa de movimento. 

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{map_distance.png}
	\caption{Representação visual dos conteúdos do mapa de distâncias\protect\footnotemark.}
	\label{map}
\end{figure}
...
A \textit{Grid} permite que um agente conheça a sua vizinhança, tornando possível usar a próximidade física entre agentes como um factor para os comportamentos entre estes. É também na \textit{Grid} que se observa o movimento dos agentes pelo espaço, contornando obstáculos e tentando atingir as saídas.
...
\subsubsection{Especificação de ficheiro de entrada}

O ficheiro a partir do qual esta ferramenta interpreta o ambiente de simulação terá como conteúdo um conjunto de linhas, todas com comprimento idêntico, compostas pelos caracteres \textit{'W'} - que representa uma parede, ou, mais genéricamente, um obstáculo -, \textit{'E'} - que representa uma saída - e {' '}, que caracteriza um espaço livre. A ordem dos caracteres nas linhas, e das linhas no ficheiro, é a ordem considerada para a representação do mapa no ambiente na simulação.

O ficheiro poderá ser selecionado na \textit{GUI} do \textit{Repast}, indo ao separador \textit{Parameters} na barra lateral, colocando o endereço para o ficheiro na caixa intitulada \textit{Environment File Name}.
